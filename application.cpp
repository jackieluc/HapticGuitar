...
//position where the audio comes from?
...
int main(int argc, char* argv[])
{
	...
	//create an audio buffer and load audio wave file
	//Note: originates from the location of project file
	...
	bool fileload1 = audioBuffer1->loadFromFile("metal-scraping.wav");
	bool fileload2 = audioBuffer2->loadFromFile("metal-impact.wav");

	//check for errors
	if (!(fileload1 && fileload2)) {
		cout << "fileload: " << fileload1 << " " << fileload2 << endl;
		cout << "Error - Sound file failed to load or initialize correctly." << endl;
		close();
		return(-1);
	}

	//convert sound to mono, which is done to support 3D sound
	audioBuffer1->convertToMono();
	audioBuffer2->convertToMono();

	//create audio source
	audioSource = new cAudioSource();
	audioSource->setSourcePos(cursor->getLocalPos());

	//assign audio buffer to audio source
	audioSource->setAudioBuffer(audioBuffer1);
	...
	//***Scene was too boring by itself
	meshBase = new cMesh();
